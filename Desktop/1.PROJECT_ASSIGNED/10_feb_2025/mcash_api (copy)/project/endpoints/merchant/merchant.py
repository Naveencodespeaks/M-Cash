...
from ...schemas.merchant import MerchantRegisterSchema, UpdateMerchantPasswordSchema 
...
from ...models.merchant import MerchantKycDetailsModel, MerchantKycDocsModel, MerchantKycDocsCommentsModel
...
router = APIRouter( prefix="/merchant", tags=["Merchant Module"], responses={404: {"description": "Not found"}},)


@router.post("/signup", response_description="Merchant Registration")
async def register(request: MerchantRegisterSchema, db: Session = Depends(get_database_session)):
    try:
        #AgentModel 
        ...
        user_with_email = db.query(MerchantModel).filter(MerchantModel.email == email).all()
        if len(user_with_email) != 0:
            return Utility.json_response(status=FAIL, message="Email already exists", error=[], data={})

        user_data = MerchantModel  (country_id=country_id,role_id =4,status_id=3, email=email,user_name=user_name, mobile_no=contact,password=AuthHandler().get_password_hash(str(password)))
        db.add(user_data)
        db.flush()
        db.commit()
...
@router.post("/login", response_description="Merchant authenticated")
async def admin_login(request: Login, db: Session = Depends(get_database_session)):
    ...
    kyc_details = relationship("MerchantKycDetailsModel", back_populates="merchant_details")
        ...
        email = request.email
        password = request.password
        user = db.query(MerchantModel).filter(MerchantModel.email == email)
        if user.count() != 1:
            return Utility.json_response(status=FAIL, message="Invalid credential's", error=[], data={})
        if user.one().status_id !=3:
            msg = "Merchant Profile is Deleted"
            if user.one().status_id == 1:
                msg = "Merchant Profile is Pending State"
            if user.one().status_id == 2:
                msg = "Merchant Profile is Pending State"
            if user.one().status_id == 4:
                msg = "Merchant Profile is Inactive State"    
            if user.one().status_id == 5:
                msg = "Merchant Profile is Delete"
        ...
        if "kyc_status_id" in user_dict and  user_data.merchant_kyc_status_details:
            user_dict["kyc_status_details"] = Utility.model_to_dict(user_data.merchant_kyc_status_details)
        if "kyc_details_id" in user_dict and  user_data.kyc_details:
            user_dict["kyc_details"] = Utility.model_to_dict(user_data.kyc_details)
        ...
        user.update({MerchantModel.token: login_token, MerchantModel.last_login:datetime.now(timezone.utc)}, synchronize_session=False)
        db.flush()
        db.commit()
...
@router.post("/update-password", response_description="Update Merchant Password")
async def reset_password(request: UpdateMerchantPasswordSchema,background_tasks: BackgroundTasks, db: Session = Depends(get_database_session)):
    try:
        user_id = request.user_id
        old_password=request.old_password
        password =  request.password
        user_obj = db.query(MerchantModel).filter(MerchantModel.id == user_id).first()
        if user_obj is None:
...
@router.post("/forgot-password", response_description="Merchant Forgot password")
async def forgot_password(request:ForgotPasswordLinkSchema,background_tasks: BackgroundTasks,db: Session = Depends(get_database_session)):
    try:
        ...
        user_obj = db.query(MerchantModel).filter(MerchantModel.email == request.email).first()
        if user_obj is None:
...
@router.post("/reset-password", response_description="Set Merchant password")
async def set_password(request:SetPasswordSchema,background_tasks: BackgroundTasks,db: Session = Depends(get_database_session)):
    try:
        ...
        user_obj = db.query(MerchantModel).filter(MerchantModel.email == user_dict["email"]).first()
        if user_obj is None:
...
async def update_kyc_details(request: UpdateKycDetails,background_tasks: BackgroundTasks,auth_user=Depends(AuthHandler().auth_wrapper), db: Session = Depends(get_database_session)):
    try:        
        ...
        #MerchantKycDetailsModel, MerchantKycDocsModel, KycDocsCommentsModel
        user_kyc_details = db.query(MerchantKycDetailsModel).filter(MerchantKycDetailsModel.merchant_id == user_id).first()
        user_obj.updated_on = datetime.now(timezone.utc)
        if user_kyc_details is None:
            details = MerchantKycDetailsModel(merchant_id=user_id,street =street,city=city, state=state,occupation_id=occupation_id,annual_income=annual_income,pincode=pincode,state_id = state_id,created_on = datetime.now(timezone.utc),updated_on = datetime.now(timezone.utc))
            ...
            if len(documents)>0:
                for document in documents:
                    ...
                    udoc = MerchantKycDocsModel(merchant_id=user_id, tenant_id=tenant_id, md_doc_id=document["md_doc_id"], name=document["name"], path=document["path"],content_type=document["content_type"],size=document["size"], status_id=2)
            ...
            if len(documents)>0:
                for document in documents:
                    ...
                    if user_obj.tenant_id:
                        masterDocQuery = masterDocQuery.filter(MdKycDocs.tenant_id==user_obj.tenant_id)
                    masterDoc = masterDocQuery.first()
                    db_doc = db.query(MerchantKycDocsModel).filter(MerchantKycDocsModel.md_doc_id == document["md_doc_id"],MerchantKycDocsModel.merchant_id==user_id).first()
...
async def update_kyc_doc_details(request: UpdateKycDocStatus,background_tasks: BackgroundTasks,auth_user=Depends(AuthHandler().auth_wrapper), db: Session = Depends(get_database_session)):
    try:
        ...
        db_doc = db.query(MerchantKycDocsModel).filter(MerchantKycDocsModel.id == document_id,MerchantKycDocsModel.merchant_id==user_id, MerchantKycDocsModel.md_doc_id==md_doc_id).first()
        if db_doc is None:
        ...
        if description:
            comment = MerchantKycDocsCommentsModel(merchant_kyc_doc_id=document_id,comment=description,commented_by=admin_id,commented_by_role_id=role_id)
            db.add(comment)
...
#kycDetailsRequest
@router.post("/get-merchant-details", response_description="KYC details")
async def kyc_details(request:kycDetailsRequest,background_tasks: BackgroundTasks,auth_user=Depends(AuthHandler().auth_wrapper), db: Session = Depends(get_database_session)):
    try:
        ...
        user_kyc_details = db.query(MerchantKycDetailsModel).filter(MerchantKycDetailsModel.merchant_id == user_id,MerchantKycDetailsModel.id == user_details["kyc_details_id"]).first()
        if user_kyc_details is None:
        ...
        db_docs = db.query(MerchantKycDocsModel).filter(MerchantKycDocsModel.merchant_id==user_id).all()
        document_id_list = []
        for doument in db_docs:
            ...
            db_doc = db.query(MerchantKycDocsModel).filter(MerchantKycDocsModel.merchant_id==user_id,MerchantKycDocsModel.id==doument.id).first()
        ...
        if len(document_id_list) > 0:
            all_comments = (db.query(MerchantKycDocsCommentsModel)
                .filter(MerchantKycDocsCommentsModel.merchant_kyc_doc_id.in_(document_id_list))
                .order_by(desc(MerchantKycDocsCommentsModel.id))
                .all())
            if all_comments:
